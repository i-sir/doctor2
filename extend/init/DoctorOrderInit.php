...
class DoctorOrderInit extends Base
{
    ...
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
            ...
            //处理时间格式
if($params['end_time'] && is_string($params['end_time'])) $params['end_time']=(strlen($params['end_time'])<=10)?strtotime($params['end_time'].' 23:59:59'):strtotime($params['end_time']);//门诊结束时间
...
if($params['pay_time'] && is_string($params['pay_time'])) $params['pay_time']=strtotime($params['pay_time']);//支付时间
          ...
          if(!empty($where)){
                ...
                $result                = $DoctorOrderModel->where($where)->strict(false)->update($params);
                //if ($result) $result = $item["id"];
            } elseif (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $DoctorOrderModel->where("id","=",$params["id"])->strict(false)->update($params);
                //if($result) $result = $item["id"];
           } else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $DoctorOrderModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if(!empty($where)){
                ...
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $DoctorOrderModel->strict(false)->insert($params);
            }
        ...
        */
        ...
        if ($type == 1) $result = $DoctorOrderModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $DoctorOrderModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $DoctorOrderModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $DoctorOrderModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
        ...
        */
        public function export_excel($where = [], $params = [])
        {
            ...
            $result       = $DoctorOrderInit->get_list($where, $params);
