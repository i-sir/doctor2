 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\DoctorCelsiusInit();
 * )
 */
...
class DoctorCelsiusInit extends Base
{
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            $result                = $DoctorCelsiusModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $DoctorCelsiusModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $DoctorCelsiusModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $DoctorCelsiusModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $DoctorCelsiusModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $DoctorCelsiusModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $DoctorCelsiusModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $DoctorCelsiusModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $DoctorCelsiusInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "温度管理"]);
    }
