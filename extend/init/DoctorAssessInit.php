...
class DoctorAssessInit extends Base
{
    ...
    */
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
          ...
          if(!empty($where)){
                ...
                $result                = $DoctorAssessModel->where($where)->strict(false)->update($params);
                //if ($result) $result = $item["id"];
            } elseif (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $DoctorAssessModel->where("id","=",$params["id"])->strict(false)->update($params);
                //if($result) $result = $item["id"];
           } else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $DoctorAssessModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if(!empty($where)){
                ...
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $DoctorAssessModel->strict(false)->insert($params);
            }
        ...
        */
        ...
        if ($type == 1) $result = $DoctorAssessModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $DoctorAssessModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $DoctorAssessModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $DoctorAssessModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
        ...
        */
        public function export_excel($where = [], $params = [])
        {
            ...
            $result       = $DoctorAssessInit->get_list($where, $params);
            ...
            $Excel->excelExports($result, $headArrValue, ["fileName" => "病情评估"]);
        }
