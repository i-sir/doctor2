 ...
 *     "version"         =>"1.0",
 *     "use"             => new \init\DoctorSymptomValueInit();
 * )
 */
...
class DoctorSymptomValueInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //症状
        if ($item['symptom_ids']) {
            $symptom_ids               = $this->getParams($item['symptom_ids']);
            $symptom_names             = $DoctorSymptomModel->where('id', 'in', $symptom_ids)->column('name');
            $item['symptom_names']     = $this->setParams($symptom_names);
            $item['symptom_name_list'] = $symptom_names;
            $item['symptom_ids']       = $symptom_ids;
        }
    ...
    public function get_list($where = [], $params = [])
    {
        ...
        /** 查询数据 **/
        $result = $DoctorSymptomValueModel
            ->where($where)
    ...
    public function edit_post($params, $where = [])
    {
        ...
        /** 公共提交,处理数据 **/
        //症状
        if ($params['symptom_ids']) {
            $symptom_ids           = array_keys($params['symptom_ids']);//提取key
            sort($symptom_ids); // 对key进行从小到大排序
            $params['symptom_ids'] = $this->setParams($symptom_ids);
            $params['symptom_ids'] = $this->setParams($symptom_ids);
        } else {
            $params['symptom_ids'] = '';
        }
        ...
        if (!empty($where)) {
            ...
            $result                = $DoctorSymptomValueModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $DoctorSymptomValueModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $DoctorSymptomValueModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $DoctorSymptomValueModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $DoctorSymptomValueModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $DoctorSymptomValueModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $DoctorSymptomValueModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $DoctorSymptomValueModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $DoctorSymptomValueInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "伴随症状结果"]);
    }
