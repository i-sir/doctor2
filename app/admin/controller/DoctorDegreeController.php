 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\DoctorDegreeController();
 * )
 */
...
class DoctorDegreeController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $where = [];
        //$where[]=["type","=", 1];
        if ($params["keyword"]) $where[] = ["name|degree|proposal", "like", "%{$params["keyword"]}%"];
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $DoctorDegreeInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $DoctorDegreeInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'DoctorDegree');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $DoctorDegreeInit->admin_edit_post($params);
    ...
    public function find()
    {
        ...
        $result                  = $DoctorDegreeInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorDegreeInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorDegreeInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $DoctorDegreeInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $DoctorDegreeInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $DoctorDegreeInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $DoctorDegreeInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
