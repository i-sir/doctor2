 ...
 *     "controller_name"     =>"DoctorSymptomValue",
 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\DoctorSymptomValueController();
 * )
 */
...
class DoctorSymptomValueController extends AdminBaseController
{
    ...
    protected function base_edit()
    {
        ...
        $symptom_map       = [];
        $symptom_map[]     = ['id', '<>', 0];
        $symptom_map[]     = ['is_show', '=', 1];
        $this->assign('symptom_list', $DoctorSymptomInit->get_list($symptom_map));
    ...
    public function index()
    {
        ...
        $where = [];
        //$where[]=["type","=", 1];
        if ($params["keyword"]) $where[] = ["name|value", "like", "%{$params["keyword"]}%"];
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $DoctorSymptomValueInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $DoctorSymptomValueInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'DoctorSymptomValue');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $DoctorSymptomValueInit->admin_edit_post($params);
    ...
    public function find()
    {
        ...
        $result                  = $DoctorSymptomValueInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorSymptomValueInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorSymptomValueInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $DoctorSymptomValueInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $DoctorSymptomValueInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $DoctorSymptomValueInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $DoctorSymptomValueInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
