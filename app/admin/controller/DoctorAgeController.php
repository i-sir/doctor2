 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\DoctorAgeController();
 * )
 */
...
class DoctorAgeController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $DoctorAgeInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $DoctorAgeInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'DoctorAge');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $DoctorAgeInit->admin_edit_post($params);
    ...
    public function find()
    {
        ...
        $result                  = $DoctorAgeInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorAgeInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $DoctorAgeInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $DoctorAgeInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $DoctorAgeInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $DoctorAgeInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $DoctorAgeInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
